...
const pdfAttachment = (fileName: string, overrides: Partial<Attachment> = {}): Attachment => ({
  base64Content: `data:application/pdf;base64,${fileName}`,
  fileName,
  fileType: 'pdf',
  fileDescription: '',
  ...overrides,
...
describe('File field input', () => {
  ...
  it('should offer to remove a document the same way it offers to remove an image', async () => {
    const user = userEvent.setup();
    await renderFileField({ ...fileValues, value: [pdfAttachment('operative-note.pdf')] });
